...
def main():
    ...
    parser.add_argument("--modality", type=str, default="ecg", choices=["ecg", "eda", "resp", "physio"])
    ...
    if args.dataset == "wesad":
        ...
        if physio_dir.exists():
            ...
            pbar = tqdm(subjs, desc="Processing StressID Subjects", unit="subject")
            for s in pbar:
                pbar.set_postfix({"subject": s, "step": "loading"})
...
def _extract_session_features(sess, modality, prov, feature_rows, pbar, s):
    ...
    stride = 5.0 if sess.dataset == "wesad" else 30.0
    specs = build_window_specs_for_session(sess, modality=modality, window_s=60.0, stride_s=stride)
